...
const TableContratos = ({getComponent}) => {
  ...
  const setIndicador = (pky,est) => {            
    ...
    dispatch({type:'CONTRATOS_INDICADOR',value:iok,estado:est})
  };
              ...
              <tbody>
                  {data.map((item) => (
                      <tr key={item.id}>                           
                        ...
                        <td><Moment format="DD-MM-YYYY">{item.fechaFin}</Moment></td>
                        <td>{item.plazo}</td>
                        <td className="text-center">
                          <FontAwesomeIcon 
                            icon={item.estado ? faCheckCircle : faTimesCircle} 
                            className={ item.estado ? "farVerdes":"farRojos" }
                          />
                        </td>                        
                      </tr>  
                      ))}
